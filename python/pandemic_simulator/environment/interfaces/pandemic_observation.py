...
class PandemicObservation:
    ...
    def create_empty(cls: Type['PandemicObservation'],
        ...
        return PandemicObservation(global_infection_summary=np.zeros((history_size, 1, len(InfectionSummary))),
                                   ...
                                   vacc_summary=np.zeros((history_size, 1, 3)),
    ...
    def update_obs_with_sim_state(self, sim_state: PandemicSimState,
        ...
        """
        num_vaxx_1 = len([p for p in sim_state.id_to_person_state.values() if p.vaccination_state>=1])
        num_vaxx_2 = len([p for p in sim_state.id_to_person_state.values() if p.vaccination_state>=2])
        num_vaxx_3 = len([p for p in sim_state.id_to_person_state.values() if p.vaccination_state>=3])
        ...
        self.vacc_summary[hist_index, 0,0] = num_vaxx_1
        self.vacc_summary[hist_index, 0,1] = num_vaxx_2
        self.vacc_summary[hist_index, 0,2] = num_vaxx_3
